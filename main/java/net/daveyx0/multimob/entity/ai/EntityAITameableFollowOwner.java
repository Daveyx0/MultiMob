...
public class EntityAITameableFollowOwner extends EntityAIBase
{
    ...
    public EntityAITameableFollowOwner(EntityLiving tameable, double followSpeedIn, float minDistIn, float maxDistIn)
    {
        ...
        if (!(tameable.getNavigator() instanceof PathNavigateGround) && !(tameable.getNavigator() instanceof PathNavigateFlying))
        {
    ...
    {
        if(this.tameable != null && this.tameable.hasCapability(CapabilityTameableEntity.TAMEABLE_ENTITY_CAPABILITY, null))
        {
        	ITameableEntity tameableEntity = EntityUtil.getCapability(tameable, CapabilityTameableEntity.TAMEABLE_ENTITY_CAPABILITY, null);
        	EntityLivingBase entitylivingbase = tameableEntity.getOwner(tameable);
    ...
    public void updateTask()
    {
        this.tameable.getLookHelper().setLookPositionWithEntity(this.owner, 10.0F, (float)this.tameable.getVerticalFaceSpeed());
            ...
            if (--this.timeToRecalcPath <= 0)
            {
                ...
                {
                    if (!this.tameable.getLeashed() && !this.tameable.isRiding() && this.tameable.getAttackTarget() == null)
                    {
