...
public class ColorUtil 
{
	...
	public static int[] getItemStackColor(ItemStack stack, World worldObj)
	{
			...
			colorMultiplier = Minecraft.getMinecraft().getItemColors().colorMultiplier(stack, 0);
    ...
    public static ResourceLocation convertToGreyScale(ResourceLocation resource)
    {
        ...
        try
        {
            BufferedImage bufferedImage = TextureUtil.readBufferedImage(resourceManager.getResource(resource).getInputStream());
            BufferedImage greyBufferedImage = new BufferedImage(bufferedImage.getWidth(), bufferedImage.getHeight(), BufferedImage.TYPE_USHORT_GRAY);
            
            Graphics g = greyBufferedImage.getGraphics();
            g.drawImage(greyBufferedImage, 0, 0, null);
            g.dispose();

            greyscaleTexture = new ResourceLocation(resource.getResourceDomain(), resource.getResourcePath());
            Minecraft.getMinecraft().getTextureManager().loadTexture(greyscaleTexture, new DynamicTexture(greyBufferedImage));
        }
        catch (IOException e)
        {
            e.printStackTrace();
