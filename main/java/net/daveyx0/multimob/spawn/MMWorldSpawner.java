...
//Additional Spawner carbon copy of vanilla one; expands spawn limit
	    ...
	    public int findChunksForSpawning(WorldServer worldServerIn, boolean spawnHostileMobs, boolean spawnPeacefulMobs, boolean spawnOnSetTickRate)
	    {
	        ...
	        else
	        {
		        ...
		        if(dimensionWhiteList != null && dimensionWhiteList.length != 0)
		        {
		        	...
		        	{
		        		if(dimensionWhiteList[i] == worldServerIn.provider.getDimension())
		        		{
		        			check = true;
		        		}
		        	}
	            ...
	            for (EnumCreatureType enumcreaturetype : EnumCreatureType.values())
	            {
	                if ((!enumcreaturetype.getPeacefulCreature() || spawnPeacefulMobs) && (enumcreaturetype.getPeacefulCreature() || spawnHostileMobs) && (!enumcreaturetype.getAnimal() || spawnOnSetTickRate))
	                {
	                    ...
	                    int l4 = (enumcreaturetype.getMaxNumberOfCreature() + MMConfigSpawns.getSpawnLimitIncrease(enumcreaturetype)) * i / MOB_COUNT_DIV;
